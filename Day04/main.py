...
def partial_overlap(data):
    count = 0

    for i in data:
        elf1_start, elf1_end = map(int, i[0].split('-'))
        elf2_start, elf2_end = map(int, i[1].split('-'))
...
def main(data):
    # count = complete_overlap(data)
    count = partial_overlap(data)
...
if __name__ == "__main__":
    input = r"C:\Users\alanv\PythonCode\Projects\Advent-of-Code-2022\Day04\input.txt"

    with open(input, "r") as f:
        data = [i.split(',') for i in f.read().splitlines()]

    count = main(data)
    # print('how many assignment pairs does one range fully contain the other? ANSWER =   {}'.format(count))
